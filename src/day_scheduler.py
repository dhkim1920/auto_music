...
class DayScheduler:
    ...
    def add_schedule(time_entry, schedule_type_var, weekday_vars):
        ...
        try:
            if schedule_type == "매주":
                DayScheduler.schedule_task(time_str=time_str)
            elif schedule_type == "평일":
                DayScheduler.schedule_task(weekdays=True, time_str=time_str)
            elif schedule_type == "주말":
                DayScheduler.schedule_task(weekends=True, time_str=time_str)
            elif schedule_type == "특정 요일" and selected_days:
                DayScheduler.schedule_task(weekdays=selected_days, time_str=time_str)
            else:
                messagebox.showwarning(title="스케줄 오류", message="유효한 스케줄을 입력하세요.")
                return
        except schedule.ScheduleValueError as e:
